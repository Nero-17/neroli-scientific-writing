...
完整论文还遵循作者明确提出的结构规则：摘要尽量简短且不超过五句话；摘要之后立即放目录；引言直接重述主要定理并沿用正文编号，同时放解释模型的图；每节一般不超过三个 subsection，绝不超过五个。合并章节应改善论述，不能只是把多余标题机械降级。

主定理后加入一个具体计算例子：明确对象和参数，实际算出定理中的量，并说明计算结果的含义。引言中的定理重述后可给精简计算，正文主定理后给完整推导；不能仅列举例子名称，也不能把有限近似写成精确极限。

Introduction 从引人入胜的具体问题、真实的猜想背景或其他数学切入点开场，自然引出本文要回答的问题。随后用 Background 交代方向进展，再给 Main theorems（沿用正文编号），最后说明 Lean formalisation 的实际范围。这是可调整的写作方式，不强制历史开场、固定标题或固定顺序；必要时可以在背景和定理之间介绍模型。没有形式化材料时应指出缺失，不能编造已验证的结论。
